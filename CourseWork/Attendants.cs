    ...
    public partial class Attendants : Form
    {
        ...
        public void populateGride()
        {
            ...
            using (SqlDataAdapter adapter = new SqlDataAdapter(cmd))
            {
                ...
                dgvAttendants.DataSource = dataTable;
            }
        }
        ...
        private void btnA_save_Click(object sender, EventArgs e)
        {
            try
            {
                string Fname = txtA_Fname.Text;
                string Lname = txtA_Lname.Text;
                DateTime DOB = Convert.ToDateTime(dpA_dob.Text);
                string nic = txtA_nic.Text;
                int ward = Convert.ToInt32(txtA_ward.Text);
        ...
        {
            if (dpA_dob.Text.Length > 0 && txtA_nic.Text.Length > 0)
            {
                SqlConnection con = new dbConnection().ConnectDB();
                string sql =
                    "INSERT INTO Attendant ([First Name], [Last Name], [DoB], [Age], NIC, [Ward]) " +
            ...
            else if (dpA_dob.Text.Length == 0 && txtA_nic.Text.Length > 0)
            {
                MessageBox.Show("Select Bithday", "Inform", MessageBoxButtons.OK, MessageBoxIcon.Information);
                dpA_dob.DataBindings.Clear();
                dpA_dob.Focus();
            }
            else if (dpA_dob.Text.Length > 0 && txtA_nic.Text.Length == 0)
            {
        ...
        private void btnA_update_Click(object sender, EventArgs e)
        {
            try
            {
                int ID = Convert.ToInt32(txtA_searchID.Text);
                string Fname = txtA_Fname.Text;
                string Lname = txtA_Lname.Text;
                DateTime DOB = Convert.ToDateTime(dpA_dob.Text);
                string nic = txtA_nic.Text;
        ...
        private void update(int id, string f, string l, DateTime d, string n, int w)
        {
            if (dpA_dob.Text.Length > 0 && txtA_nic.Text.Length > 0)
            {
                SqlConnection con = new dbConnection().ConnectDB();
                string sql =
                    "UPDATE Attendant SET [First Name] = @Fname, [Last Name] = @Lname, DoB = @dob, [NIC] = @NIC, [Ward] = @Ward WHERE ID = @ID;";
        ...
        private void btnA_search_Click(object sender, EventArgs e)
        {
            string NIC = txtA_searchNIC.Text;
            if (int.TryParse(txtA_searchID.Text, out int ID))
            {
                search(NIC, ID);
            }
            else
        ...
        private void search(string nic, int id)
        {
            ...
            {
                string sql = "SELECT * FROM Attendant WHERE NIC = @NIC";
            ...
            {
                string sql = "SELECT * FROM Attendant WHERE ID = @ID";
                ...
                using (SqlCommand cmd = new SqlCommand(sql, con))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            txtA_Fname.Text = reader["First Name"].ToString();
                            txtA_Lname.Text = reader["Last Name"].ToString();
                            dpA_dob.Text = reader["DoB"].ToString();
                            txtA_nic.Text = reader["NIC"].ToString();
                            txtA_ward.Text = reader["Ward"].ToString();
                        }
                        else
                        {
                            MessageBox.Show("Record not found.");
                            clearTextBoxesAndDatePicker();
                            txtA_searchID.Text = "0";
                        }
                    }
                }
            }
        ...
        private void btnA_return_Click(object sender, EventArgs e)
        {
            populateGride(); 
            clearTextBoxesAndDatePicker();
            txtA_searchID.Text = "0";
